...
export class DefaultApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param levelId 
     */
    public async getCwmsDataLevelsWithLevelIdTimeseries(levelId: string, _options?: Configuration): Promise<RequestContext> {
        let _config = _options || this.configuration;

        // verify required parameter 'levelId' is not null or undefined
        if (levelId === null || levelId === undefined) {
            throw new RequiredError("DefaultApi", "getCwmsDataLevelsWithLevelIdTimeseries", "levelId");
        }
...
export class DefaultApiResponseProcessor {
     ...
     */
     public async getCwmsDataWithHttpInfo(response: ResponseContext): Promise<HttpInfo< void>> {
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getCwmsDataLevelsWithLevelIdTimeseriesWithHttpInfo(response: ResponseContext): Promise<HttpInfo< void>> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("500", response.httpStatusCode)) {
            const body: CdaError = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getCwmsDataTimeseriesRecentWithGroupIdWithHttpInfo(response: ResponseContext): Promise<HttpInfo< void>> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("500", response.httpStatusCode)) {
            const body: CdaError = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
