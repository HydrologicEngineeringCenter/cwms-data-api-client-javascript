...
export class PoolsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     */
    public async getCwmsDataPools(office?: string, idMask?: string, nameMask?: string, bottomMask?: string, topMask?: string, includeExplicit?: string, includeImplicit?: string, page?: string, pageSize?: number, _options?: Configuration): Promise<RequestContext> {
...
export class PoolsApiResponseProcessor {
     ...
     */
     public async getCwmsDataPoolsWithHttpInfo(response: ResponseContext): Promise<HttpInfo<Pools >> {
        ...
        if (isCodeInRange("404", response.httpStatusCode)) {
            ...
            throw new ApiException<CdaError>(response.httpStatusCode, "Based on the combination of inputs provided the pools were not found.", body, response.headers);
        }
     ...
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async getCwmsDataPoolsWithPoolIdWithHttpInfo(response: ResponseContext): Promise<HttpInfo<Pool >> {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("500", response.httpStatusCode)) {
            const body: CdaError = ObjectSerializer.deserialize(
                ObjectSerializer.parse(await response.body.text(), contentType),
        ...
        if (isCodeInRange("404", response.httpStatusCode)) {
            ...
            throw new ApiException<CdaError>(response.httpStatusCode, "Based on the combination of inputs provided the Location Category was not found.", body, response.headers);
        }
        if (isCodeInRange("200", response.httpStatusCode)) {
